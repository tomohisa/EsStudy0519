...
using EsStudy0519.Domain.Aggregates.WeatherForecasts.Queries;
using EsStudy0519.Domain.Generated;
using EsStudy0519.Domain.Projections.Count;
using ResultBoxes;
...
builder.AddKeyedAzureTableClient("EsStudy0519Clustering");
builder.AddKeyedAzureTableClient("EsStudy0519GrainTable");
builder.AddKeyedAzureBlobClient("EsStudy0519GrainState");
builder.AddKeyedAzureQueueClient("EsStudy0519Queue");
builder.UseOrleans(config =>
{
            ...
            configurator =>
            {
                ...
                configurator.UseAzureTableCheckpointer(ob => ob.Configure(cp =>
                {
                    ...
                    cp.ConfigureTableServiceClient(
                        builder.Configuration.GetConnectionString("EsStudy0519Table"));
                }));
            });
        config.AddEventHubStreams(
            "EsStudy0519Queue",
            configurator =>
            {
                ...
                configurator.UseAzureTableCheckpointer(ob => ob.Configure(cp =>
                {
                    cp.TableName = "EventHubCheckpointsEsStudy0519Queue"; // any table name you like
                    cp.PersistInterval = TimeSpan.FromSeconds(10); // write frequency
                    cp.ConfigureTableServiceClient(
                        builder.Configuration.GetConnectionString("EsStudy0519Table"));
                }));
    ...
    else
    {
        config.AddAzureQueueStreams("EventStreamProvider", configurator =>
        {
            configurator.ConfigureAzureQueue(options =>
            {
                ...
                {
                    queueOptions.QueueServiceClient = sp.GetKeyedService<QueueServiceClient>("EsStudy0519Queue");
        ...
        config.AddAzureQueueStreams("EsStudy0519Queue", configurator =>
        {
            configurator.ConfigureAzureQueue(options =>
            {
                options.Configure<IServiceProvider>((queueOptions, sp) =>
                {
                    ...
                    queueOptions.QueueNames =
                    [
                        "ywnh5ws65snztguqv8zfa3raz-esstudy0519queue-0",
                        "ywnh5ws65snztguqv8zfa3raz-esstudy0519queue-1",
                        "ywnh5ws65snztguqv8zfa3raz-esstudy0519queue-2"
                    ];
                    queueOptions.MessageVisibilityTimeout = TimeSpan.FromMinutes(2);
                });
            });
    ...
    else
    {
        config.AddAzureTableGrainStorage("PubSubStore", options =>
        {
            ...
            {
                opt.TableServiceClient = sp.GetKeyedService<TableServiceClient>("EsStudy0519GrainTable");
