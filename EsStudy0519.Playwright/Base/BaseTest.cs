...
namespace EsStudy0519.Playwright.Base
{
    public class BaseTest : IDisposable
    {
        ...
        public async Task SetUp()
        {
            ...
            try
            {
                // Kill any existing EsStudy0519 processes first
                KillEsStudy0519Processes();
                ...
                {
                    throw new DirectoryNotFoundException("Could not find EsStudy0519.AppHost directory in any of the expected locations");
                }
        ...
        public async Task TearDown()
        {
            ...
            // Kill EsStudy0519 processes
            Console.WriteLine("Killing EsStudy0519 processes...");
            var killSw = Stopwatch.StartNew();
            
            KillEsStudy0519Processes();
            
            killSw.Stop();
            Console.WriteLine($"EsStudy0519 processes killed in {killSw.ElapsedMilliseconds}ms");
        }
        ...
        private string FindAppHostDirectory(string startingDirectory)
        {
            ...
            {
                "../../EsStudy0519.AppHost",                           // From bin/Debug/net9.0
                "../../../EsStudy0519.AppHost",                        // From bin/Debug
                "../../../../EsStudy0519.AppHost",                     // From bin
                "../../../../../EsStudy0519.AppHost",                  // From Playwright
                "../../..",                                                 // To solution root
                "../../../.."                                               // One level up from solution root
            };
            
            // First try to find the solution root by looking for EsStudy0519.sln
            string solutionRoot = FindSolutionRoot(startingDirectory);
            
            if (!string.IsNullOrEmpty(solutionRoot))
            {
                string appHostPath = Path.Combine(solutionRoot, "EsStudy0519.AppHost");
            ...
            foreach (string relativePath in possiblePaths)
            {
                try
                {
                    ...
                    if (Directory.Exists(fullPath))
                    {
                        ...
                        if (Path.GetFileName(fullPath) == "EsStudy0519.AppHost")
                        {
                            return fullPath;
                        }
                        
                        // Check if AppHost is a subdirectory
                        string appHostSubdir = Path.Combine(fullPath, "EsStudy0519.AppHost");
            ...
            try
            {
                ...
                {
                    string appHostPath = Path.Combine(projectRoot, "EsStudy0519.AppHost");
                    if (Directory.Exists(appHostPath))
                    {
                        return appHostPath;
                    }
        ...
        private void KillEsStudy0519Processes()
        {
            try
            {
                ...
                // Use pkill to kill all EsStudy0519 processes
                var pkillProcess = new Process
                {
                    StartInfo = new ProcessStartInfo
                    {
                        FileName = "pkill",
                        Arguments = "-9 -f \"EsStudy0519.\"",
                ...
                Console.WriteLine("All EsStudy0519 processes killed");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error in KillEsStudy0519Processes: {ex.Message}");
            }
        }
